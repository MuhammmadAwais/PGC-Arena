...
export function CloudinaryUploadZone({
  ...
  // Handle direct file drop / browse upload fallback
  const handleDirectFile = async (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (!file) return;

    if (file.size > 10 * 1024 * 1024) {
      setDirectError("File exceeds 10MB limit.");
      return;
    }
    ...
    try {
      ...
      setDirectError(err.message || "Failed to upload image.");
    } finally {
    ...
    <div className="space-y-2 font-sans">
      ...
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          className="text-[10px] text-slate-400 hover:text-cyan-300 font-medium flex items-center gap-1 transition-colors group cursor-pointer"
        >
          <LinkIcon className="w-2.5 h-2.5 text-slate-500 group-hover:text-cyan-300 transition-colors" />
          <span>{showUrlInput ? "Cancel URL" : "Paste URL"}</span>
        </button>
      </div>
      ...
      {value ? (
        ...
        <div
          className={`relative rounded-2xl border border-white/[0.15] bg-black/60 overflow-hidden shadow-lg group backdrop-blur-md transition-all ${
            variant === "compact" ? "h-20 w-20" : "h-36 w-full"
          }`}
        >
          <img
            src={value}
          ...
          <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/40 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center gap-2.5 p-3">
            <CldUploadWidget
          ...
          {/* Persistent subtle top-right action pills on mobile/quick view */}
          <div className="absolute top-2 right-2 flex items-center gap-1.5 opacity-90 group-hover:opacity-0 transition-opacity">
            <CldUploadWidget
              uploadPreset={uploadPreset}
              options={{
                maxFiles: 1,
            ...
            >
              {({ open }) => (
                <button
                  ...
                  onClick={() => open()}
                  title="Change Image"
                  className="p-1.5 rounded-lg bg-black/70 backdrop-blur-md border border-white/20 text-slate-300 hover:text-white transition-colors"
                >
            ...
            {onRemove && (
              <button
                ...
                onClick={onRemove}
                title="Remove Image"
                className="p-1.5 rounded-lg bg-black/70 backdrop-blur-md border border-pgc-red/30 text-pgc-red hover:bg-pgc-red/30 transition-colors"
              >
        ...
        >
          {({ open }) => (
            <div
              className={`relative rounded-2xl border-2 border-dashed border-white/15 hover:border-cyan-400/70 bg-white/[0.02] hover:bg-cyan-500/[0.04] transition-all duration-300 cursor-pointer flex flex-col items-center justify-center p-4 text-center group ${
                variant === "compact" ? "h-20 w-20 p-2" : "h-36 w-full"
            ...
            >
              {isDirectUploading ? (
                <div className="flex flex-col items-center gap-2">
                  ...
                  <span className="text-[11px] font-mono text-slate-300">Uploading to Cloudinary...</span>
                </div>
              ) : (
                <>
                  <div className="w-10 h-10 rounded-xl bg-white/[0.04] group-hover:bg-cyan-500/20 text-slate-400 group-hover:text-cyan-300 border border-white/10 group-hover:border-cyan-500/40 flex items-center justify-center transition-all duration-300 mb-2 shadow-sm group-hover:scale-110">
                    {variant === "banner" ? (
                  ...
                  <p className="text-xs font-bold text-slate-300 group-hover:text-white transition-colors">
                    {variant === "banner" ? "Upload Banner Image" : "Upload Emblem / Avatar"}
                  </p>
                  <p className="text-[10px] text-slate-500 group-hover:text-slate-400 mt-0.5 transition-colors">
      ...
      {directError && (
        <p className="text-[11px] text-pgc-red font-mono">{directError}</p>
      )}
    </div>
  );
}
