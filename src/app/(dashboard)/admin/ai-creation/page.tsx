...
export const metadata: Metadata = {
  title: "AI Question Forge — PGC Arena Admin",
  description: "Generate and seed questions using the Gemini AI pipeline from uploaded PDFs.",
};

const pipeline = [
  { step: "1", label: "Upload PDF", icon: Upload, done: false, desc: "Drop a past paper or textbook chapter" },
  { step: "2", label: "AI Parsing", icon: Brain, done: false, desc: "Gemini extracts and structures questions" },
  { step: "3", label: "Review & Edit", icon: CheckCircle, done: false, desc: "Human review before committing to bank" },
];
    ...
    <div className="flex flex-col gap-8">
      <div>
        ...
        <p className="mt-1 text-sm text-white/45">
          Upload PDFs and let the Gemini pipeline automatically extract, structure, and seed questions into the vault.
        </p>
      </div>
        ...
        {pipeline.map(({ step, label, icon: Icon, desc }) => (
          ...
          >
            <div className="flex items-center gap-3 mb-3">
              ...
              <span className="text-[10px] font-bold text-white/30 uppercase tracking-widest">Step {step}</span>
            </div>
      ...
      <div className="rounded-2xl p-12 min-h-[320px] flex flex-col items-center justify-center gap-4 bg-white/[0.03] border border-white/[0.08] border-dashed hover:border-pgc-red/30 hover:bg-pgc-red/[0.02] transition-all duration-300 cursor-pointer">
        ...
        <div className="text-center">
          <p className="text-sm font-semibold text-white/40">PDFDropzone component</p>
          <p className="text-xs text-white/25 mt-1.5 leading-relaxed">
            Drag &amp; drop a past paper PDF here. The AI pipeline (
            <span className="text-pgc-red/60 font-medium">parsePdfWithGemini</span>) will extract MCQs,
            structured questions, and fill-in-the-blank items automatically.
          </p>
        </div>
        <p className="text-xs text-white/15 mt-2">Route: /admin/ai-creation · Feature: ai-seeding</p>
      </div>
    </div>
  );
}
